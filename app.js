...
d3.csv("hairData.csv", function(err, hairData) {
  ...
  hairData.forEach(function(data) {
    data.teeth_removed = +data.teeth_removed;
    data.education_hits = +data.education_hits;
    data.visit_doc = +data.visit_doc;
  });
  ...
  function findMinAndMax(dataColumnX) {
    ...
    yMax = d3.max(hairData, function(data) {
      return +data.education_hits * 1.1;
    });
  }
  ...
  var currentAxisLabelX = "teeth_removed";
  ...
  var toolTip = d3
    ...
    .html(function(data) {
      ...
      var numHits = +data.education_hits;
      var bandInfo = +data[currentAxisLabelX];
      var bandString;
      // Tooltip text depends on which axis is active/has been clicked
      if (currentAxisLabelX === "teeth_removed") {
        bandString = "Teeth removed: ";
      }
      else {
        bandString = "Visited doctor: ";
      }
      return bandName +
        "<br>" +
        bandString +
  ...
  chart
    ...
    .attr("cy", function(data, index) {
      return yLinearScale(data.education_hits);
    })
    .attr("r", "15")
    .attr("fill", "#50C2E3")
    ...
    .append("tspan")
        .attr("x", function(data) {
            return xLinearScale(+data[currentAxisLabelX]-0);
        })
        .attr("y", function(data) {
            return yLinearScale(data.education_hits-0.2);
        })
        .attr("class", "abbr")
        .text(function(data) {
            return data.abbr
  ...
  chart
    ...
    .attr("data-axis-name", "education_hits")
    .text("% +65 year old with Education (Bachelor or Higher)");
  ...
  chart
    ...
    .attr("data-axis-name", "teeth_removed")
    .text("% +65 year old that had all teeth removed");
  ...
  chart
    ...
    .attr("data-axis-name", "visit_doc")
    .text("% Visited doctor in the past 2 yrs");
